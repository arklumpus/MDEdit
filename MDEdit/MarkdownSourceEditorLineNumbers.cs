...
namespace MDEdit
{
    internal class MarkdownSourceEditorLineNumbers : Control, ILogicalScrollable
    {
        ...
        private static readonly Brush LineNumberBackgroundBrush = new SolidColorBrush(Color.FromRgb(245, 245, 245));

        public override void Render(DrawingContext context)
        {
            base.Render(context);

            context.FillRectangle(LineNumberBackgroundBrush, new Rect(0, 0, this.Editor.LineNumbersWidth + 15, this.Bounds.Height));
            ...
            using (context.PushClip(new Rect(0, 0, this.Bounds.Width, this.Bounds.Height)))
            {
                ...
                for (int i = firstLine; i <= lastLine; i++)
                {
                    ...
                    context.DrawText(this.Editor.LineNumbersBrush, new Point(5 + this.Editor.LineNumbersWidth - (Math.Floor(Math.Log10(i + 1)) + 1) * this.Editor.CharacterWidth, i * lineHeight - this.Offset.Y), formattedText);
                }
                ...
                if (this.Editor.ShowLineChanges)
                {
                    foreach (HighlightedLineRange lines in this.Editor.HighlightedLines)
                    {
                        if (lines.LineSpan.End >= firstLine && lastLine > lines.LineSpan.Start - firstLine)
                        {
                            ...
                            context.FillRectangle(lines.HighlightBrush, new Avalonia.Rect(5 + this.Editor.LineNumbersWidth + 5, topY, 5, bottomY - topY));
                        }
                    }
                }
            }
