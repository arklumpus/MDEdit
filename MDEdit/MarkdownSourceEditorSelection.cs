...
namespace MDEdit
{
    internal class MarkdownSourceEditorSelection : Control, ILogicalScrollable
    {
        ...
        public override void Render(DrawingContext context)
        {
                ...
                for (int i = startPosition.Line; i <= endPosition.Line; i++)
                {
                    ...
                    if (i >= firstLine && i <= lastLine)
                    {
                        if (i == startPosition.Line)
                        {
                            ...
                            {
                                rectsToFill.Add(MarkdownSourceEditorCaret.Round(new Rect(Math.Max(0, startPosition.Character - firstColumn) * this.Editor.CharacterWidth - this.Offset.X % this.Editor.CharacterWidth + 25 + this.Editor.LineNumbersWidth, i * lineHeight - this.Offset.Y, (Math.Min(endPosition.Line != startPosition.Line ? int.MaxValue : endPosition.Character, Math.Min(lastColumn + 1, line.End - line.Start + 1)) - Math.Max(firstColumn, startPosition.Character)) * this.Editor.CharacterWidth, lineHeight), new Size(1, 1)));
                            }
                        }
                        else if (i == endPosition.Line && endPosition.Line != startPosition.Line)
                        {
                            ...
                            {
                                rectsToFill.Add(MarkdownSourceEditorCaret.Round(new Rect(-this.Offset.X % this.Editor.CharacterWidth + 25 + this.Editor.LineNumbersWidth, i * lineHeight - this.Offset.Y, (Math.Min(lastColumn + 1, selEnd - line.Start) - firstColumn) * this.Editor.CharacterWidth, lineHeight), new Size(1, 1)));
                            }
                        }
                        else
                        {
                            if (firstColumn < line.End - line.Start + 1)
                            {
                                rectsToFill.Add(MarkdownSourceEditorCaret.Round(new Rect(-this.Offset.X % this.Editor.CharacterWidth + 25 + this.Editor.LineNumbersWidth, i * lineHeight - this.Offset.Y, (Math.Min(lastColumn + 1, line.End - line.Start + 1) - firstColumn) * this.Editor.CharacterWidth, lineHeight), new Size(1, 1)));
                            }
                        }
                    }
                }
            ...
            foreach (MarkerRange range in this.Editor.Markers)
            {
                ...
                {
                    Point startPoint = new Point(span.Start.Character * this.Editor.CharacterWidth - this.Offset.X + 25 + this.Editor.LineNumbersWidth, span.Start.Line * lineHeight - this.Offset.Y + this.Editor.FontSize * 1.3);
            ...
            if (this.Editor.SearchSpans.Count > 0 && this.Editor.SearchReplace.IsVisible)
            {
                foreach (SearchSpan matchSpan in this.Editor.SearchSpans)
                {
                    foreach (LinePositionSpan span in matchSpan.Span.ToLinePositionSpans(this.Editor.Text))
                    {
                        ...
                        {
                            context.FillRectangle(SearchBrush, MarkdownSourceEditorCaret.Round(new Rect(span.Start.Character * this.Editor.CharacterWidth - this.Offset.X + 25 + this.Editor.LineNumbersWidth, span.Start.Line * lineHeight - this.Offset.Y, (span.End.Character - span.Start.Character) * this.Editor.CharacterWidth, lineHeight), new Size(1, 1)));
                        }
                    }
                }
            }
