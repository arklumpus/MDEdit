...
namespace MDEdit
{
    internal class MarkdownSourceEditorCaret : Control, ILogicalScrollable
    {
        ...
        public Rect GetCaretRectangle()
        {
            ...
            if (!Editor.OverstrikeMode || Editor.CaretOffset == Editor.Text.Lines[line].End)
            {
                return Round(new Rect((character - firstColumn) * Editor.CharacterWidth - this.Offset.X % Editor.CharacterWidth + 25 + this.Editor.LineNumbersWidth, line * lineHeight - this.Offset.Y, 0.5, lineHeight), new Size(1, 1));
            }
            else
            {
                return Round(new Rect((character - firstColumn) * Editor.CharacterWidth - this.Offset.X % Editor.CharacterWidth + 25 + this.Editor.LineNumbersWidth, line * lineHeight - this.Offset.Y, Editor.CharacterWidth, lineHeight), new Size(1, 1));
            }
        }
